...
public class TfLOverlayManager implements MapEventsReceiver {
    ...
    public void updateMarkerOfNearest(Station nearest) {
        ...
        if (nearest != null) {
            for(Marker m: this.stnClusterer.getItems()){
                if (nearest.equals(m.getRelatedObject())){
    ...
    public void onStationMarkerLongPress(Station longPressedStn) {
        Station stn = StationManager.getInstance().getStationWithId(longPressedStn.getID());
        ResourceProxy rp = new DefaultResourceProxyImpl(mapFragment.getActivity());
        for (Line l: stn.getLines()) {

            Polyline polyLine = new Polyline(rp);
            ...
            for (Branch b : l.getBranches()) {
                ...
                for (LatLon latLon : b.getPoints()) {
                    GeoPoint geo = new GeoPoint(latLon.getLatitude(), latLon.getLongitude());
    ...
    public boolean longPressHelper(GeoPoint geoPoint) {
        boolean shouldIEarse = true;
        for (Station stn: StationManager.getInstance()){
            GeoPoint geoPoint1 = new GeoPoint(stn.getLocn().getLatitude(), stn.getLocn().getLongitude());
            if(geoPoint.equals(geoPoint1)){
                shouldIEarse = false;
                break;
            }
        }
        if(shouldIEarse) {
            this.tubeLineOverlays.clear();
            updateOverlays();
        }
    ...
    private void markStations() {
        ...
        for (Station stn: StationManager.getInstance()){
            ...
            stationMarker.setTitle(stn.getName());
            ...
            this.stnClusterer.add(stationMarker);
        }
        updateOverlays();
    }
    ...
    private void plotLines() {
        for (Polyline p: this.tubeLineOverlays){
            p.setVisible(true);
        }
        updateOverlays();
    }
